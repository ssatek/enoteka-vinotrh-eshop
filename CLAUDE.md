...
- **Spuštění:** `py -3 src/transform.py` → zapíše `output/wines.json` (120 vín) a `output/k_doplneni.json` (report položek k doplnění, aktuálně jen 7× chybějící `Typ cukernatosti`).
...
## Testování
- **Nikdy needituj `index.html`/`detail.html` dvojklikem (`file://` protokol) — data se nenačtou.** `fetch()` na `output/wines.json` je prohlížečem blokovaný přes `file://` schéma (`URL scheme "file" is not supported"`), karty zůstanou prázdné. Ověřeno Playwrightem — 0 karet, `TypeError: Failed to fetch`. Na produkci (Vercel, skutečné HTTP) to problém není, týká se to jen lokálního otevření souboru.
- Lokálně vždy přes HTTP server: `npx http-server -p 5714 -c-1 .` → otevřít `http://localhost:5714/index.html` (ne `npx serve` — ten defaultně dělá "clean URL" redirect, který při přesměrování `/detail.html?pozice=1` → `/detail` ztrácí query string; na produkci na Vercelu k tomu nedochází, protože `vercel.json` cleanUrls nezapíná).
